...
; New as January 4, 2011. The old version affected the inferior-lisp-mode-hook in an inappropriate way.
(defun run-laml-interactively ()
  "Start an interactive Scheme process which in addition possess the LAML context information
of the directory, from which it has been started. In addition, it loads the LAML fundamental setup
library, the general library, the file-read library, the time library, and a HTML/XML mirror library.
...
Runs the hook run-laml-interactively-hook."
  ...
  (let ((inferior-lisp-mode-hook (cons 'lamlify-scheme-process inferior-lisp-mode-hook)))   ; avoid global effects on inferior-lisp-mode-hook
...
(defun laml-kind-of-distribution ()
  "Return the kind of LAML distribution as of this installation. Either full, slim, schemedoc, or development. A symbol."
  (let ((distribution-version-list (file-read (concat laml-dir "distribution-version.lsp"))))
    (as-symbol (nth 3 distribution-version-list))))

(defun laml-version-of-distribution ()
  "Return a list of two numbers (main-version sub-version), such as (37 0), reflecting the LAML version."
  (let ((distribution-version-list (file-read (concat laml-dir "distribution-version.lsp"))))
    (list (car distribution-version-list) (cadr distribution-version-list))))
