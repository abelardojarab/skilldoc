 ...
 (let ((local-manual-source-list #f))
  ...
  (let* ((source-file-path-syntactical-comments (in-laml-temp "temp-source-file.scm"))
   ...
   (let ((source-list (read-source source-file-path-syntactical-comments source-key version-number)))
     ...
     (set! render-documentation-comments-in-source-programs #f)   ; no level 2 comments in rendered source programs.
                                                                  ; Doxygen inspirred simplification of rendered source programs.
...
(define (is-testcase-about-doc-entry testcase doc-entry-name)
 (let ((example-marker (test-case-example-info-of testcase)))
  (or (and (eq? (test-case-type-of testcase) 'documentation-case)
           (is-testcase-about testcase doc-entry-name))
...
(define (make-example-from-testcase testcase)
 (cond ((eq? (test-case-type-of testcase) 'error-functional-testcase)
       ...
       ((eq? (test-case-type-of testcase) 'documentation-case)
              (list 'example
		    (string-append (real-test-case-expression-of testcase) "<br>" (documentation-case-documentation-of testcase) )))

       (else (laml-error "make-example-from-testcase: Unknown type of test case"  (test-case-type-of testcase)))))
...
(define (manual-index-order-leq? x y)
  (string-ci<=? (cadr (assq 'title x)) (cadr (assq 'title y))))
  ...
  (tr 'class "manual-row"
    (td 'class left-class 'rowspan (as-string rowspan)
        ...
        (if kind-explanation
            ...
            '()))
    (td  'class right-class 'colspan "3" right-contribution))))   ; colspan 2 corrected to 3 December 23, 2008.
